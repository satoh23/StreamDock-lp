...
const HIGHLIGHTS: Highlight[] = [
  ...
  },
  {
    ...
    // ⚠ 素材は 0.4 秒から 4.5 秒ぶんを切ってある。アニメは 0.5 秒に始まり
    //   3.6 秒で「大当たり」に達して以降は静止するので、静止しているところで
    //   始めて静止しているところで終わる＝ループの継ぎ目が目立たない。
    // ⚠ ポスターは「大当たり」の瞬間（3.9 秒）から取る。先頭から取ると
    //   ガチャマシンが出る前の静止画になり、何の機能か分からない。
    // ⚠ 切り出しは**背景透過のエリアだけ**（元画像の x76..1031 / y810..1347）。
    //   枠線・再生ボタン・説明文は入れない（依頼者の指示）。
    //   拡大せずに済むよう 956x538 のまま置く（表示幅 453px・Retina でも足りる）。
    asset: {
      name: "cv-gacha",
      width: 956,
      height: 538,
